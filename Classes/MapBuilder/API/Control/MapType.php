 ...
 */
class Tx_AdGoogleMaps_MapBuilder_API_Control_MapType extends Tx_AdGoogleMaps_MapBuilder_API_Control_AbstractControl {
	...
	public function setMapTypeIds($mapTypeIds) {
		if (is_array($mapTypeIds) === FALSE) {
			$mapTypeIds = Tx_Extbase_Utility_Arrays::trimExplode(',', $mapTypeIds);
		}
	...
	public function hasMapTypeIds() {
		return count($this->mapTypeIds) > 0;
	}
	...
	public function hasOptions() {
		return ($this->hasMapTypeIds() === TRUE || $this->position !== self::POSITION_TOP_RIGHT || $this->style !== self::STYLE_DEFAULT);
	}
