...
abstract class Tx_AdGoogleMaps_MapDrawer_Layer_AbstractLayer {
	...
	public function __construct($currentField, $formObject, $recordTypeConfiguration) {
		...
		if (Tx_AdGoogleMapsApi_Api_LatLng::isValidCoordinate($center) === FALSE) {
			throw new Tx_AdGoogleMaps_MapDrawer_Exception('Given map center "' . $center . '" is invalid. The format must be like "48.209206,16.372778".<br />See: plugin.tx_adgooglemapsapi.settings.api.center', 1299154340);
		} else {
			$center = t3lib_div::makeInstance('Tx_AdGoogleMapsApi_Api_LatLng', $center);
		}
