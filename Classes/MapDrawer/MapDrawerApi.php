...
class tx_AdGoogleMaps_MapDrawer_MapDrawerApi {
	...
	public function tx_draw($currentField, $formObject) {
		...
		if (($settings = Tx_AdGoogleMapsApi_Utility_BackEnd::loadSettings($TSCpid, 'tx_adgooglemaps')) === FALSE || array_key_exists('mapDrawer', $settings) === FALSE) {
			return $this->addFlashMessage('Add static TypoScript "ad: Google Maps Api" to your template.');
		}
		$this->settings = $settings['mapDrawer'];

		// Check table configuration.
		if (array_key_exists($currentField['table'], $this->settings['tables']) === FALSE) {
			return $this->addFlashMessage('MapDrawer found no configuration for current table "' . $currentField['table'] . '".<br />See: plugin.tx_adgooglemaps.settings.mapDrawer.tables.*');
		}
		...
		if (array_key_exists($tableSettings['typeField'], $currentField['row'])) {
			...
			return $this->addFlashMessage('MapDrawer found no record type configuration for current table "' . $currentField['table'] . '".<br />See: plugin.tx_adgooglemaps.settings.mapDrawer.tables.typeField');
		}
		...
		if (class_exists($recordTypeConfiguration['className']) === FALSE) {
			$this->addFlashMessage('MapDrawer found no record type configuration for current type "' . $recordType . '".<br />See: plugin.tx_adgooglemaps.settings.mapDrawer.tables.recordTypes');
	 ...
	 * @return void
	 */
	protected function addFlashMessage($message, $title = 'tx_adgooglemaps: Invalid extension configuration', $severity = t3lib_FlashMessage::ERROR) {
