	...
	public static function includeFrontEndResources($configurationKey) {
		if (array_key_exists($configurationKey, self::$loadedFrontEndResources) === FALSE && ($settings = Tx_AdGoogleMaps_Utility_BackEnd::getTypoScriptSetup($GLOBALS['TSFE']->id, 'tx_adgooglemaps')) !== FALSE) {
			if (array_key_exists($configurationKey, $settings['plugin']['includeFrontEndResources']) !== FALSE) {
				...
				$resources = class_exists('Tx_Extbase_Service_TypoScriptService')
					? t3lib_div::makeInstance('Tx_Extbase_Service_TypoScriptService')->convertPlainArrayToTypoScriptArray($settings['plugin']['includeFrontEndResources'][$configurationKey])
					: Tx_Extbase_Utility_TypoScript::convertPlainArrayToTypoScriptArray($settings['plugin']['includeFrontEndResources'][$configurationKey]);
