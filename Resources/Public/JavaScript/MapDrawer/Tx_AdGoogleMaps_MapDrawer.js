...
TYPO3.Tx_AdGoogleMaps_MapDrawer_AbstractLayer = Ext.extend(Object, {
	...
	markerIcon: {
		'address': new google.maps.MarkerImage(
			'../typo3conf/ext/ad_google_maps_api/Resources/Public/Icons/MapDrawer/searchMarker.gif', 
	...
	createMap: function(){
		...
		if (canvas === null){
			alert('Error TYPO3.Tx_AdGoogleMapsApi_MapDrawerApi.createMap(): Map container with ID "' + this.canvasId + '" not found.');
		} else {
		...
		this.geocoder.geocode({ 'address': address }, function(results, status){
			if (status == google.maps.GeocoderStatus.OK) {
				var searchMarkerInfoContent = 
					...
					'<input type="button" onclick="javascript: ' + _this.objectId + '.searchMarkerCallback(new google.maps.LatLng(' + results[0].geometry.location.toUrlValue() + ')); return false;" value="Set Marker" /">';
